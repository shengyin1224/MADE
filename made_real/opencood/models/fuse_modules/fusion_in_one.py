...
class AttFusion(nn.Module):
    def __init__(self, feature_dims, temperature=20):
        super(AttFusion, self).__init__()
        self.att = ScaledDotProductAttention(feature_dims, temperature)

    def forward(self, xx, record_len, pairwise_t_matrix, if_fuse = True, num = 0, 
                if_draw = False, cls = None, if_att_score = False):
        _, C, H, W = xx.shape
        B, L = pairwise_t_matrix.shape[:2]
        ...
        for b in range(B):
            ...
            h, attention_score = self.att(x, x, x, num, if_draw, cls, if_att_score)
