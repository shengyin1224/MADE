...
from opencood.models.shift import Shift
from opencood.models.rotate import Rotate
...
class Where2comm(nn.Module):
    ...
    def forward(self, x, rm, record_len, pairwise_t_matrix, backbone=None, data_dict = None, attack = None, 
                ...
                no_att_output_dict = None, dataset = None, shift_dir_of_box = []):
        """
        Fusion forwarding.
        ...
        if self.multi_scale:
            ...
            for i in range(self.num_levels):
                ...
                if shift_feature and i == 0 and x.shape[0] > 1:
                    ...
                    shift_model = Shift(tmp_model, **attack_conf.attack.shift)

                    tmp_att, _ = shift_model(real_data_dict, data_dict, if_attack_feat = True, attack_feat = attacked_feature_list, if_erase = if_erase, erase_index = erase_index, num = num, attack_conf = attack_conf, attack_target = attack_target, no_att_output_dict = no_att_output_dict, dataset = dataset, shift_dir_of_box = shift_dir_of_box)
                ...
                #         tmp_residual_vector = (x_fuse - x[k]).detach().cpu().numpy()
                #         np.save(f'/GPFS/data/shengyin/OpenCOOD-main/outcome/residual_vector_all/new_0322/sample_{num}_agent_{k}.npy', tmp_residual_vector)
        ...
        return x_fuse, communication_rates, {}, residual_vector
