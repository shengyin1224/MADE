...
class ResNetBEVBackbone(nn.Module):
    ...
    def decode_multiscale_feature(self, x):
        ...
        for i in range(self.num_levels):
            if len(self.deblocks) > 0:
                #tmp = self.deblocks[i](x[i])
                ups.append(self.deblocks[i](x[i]))

                # 存储decoder后的结果
                # path = 'outcome/fused_feature_list' + '/no_temp_[0,1,2]/'
                # if os.path.exists(path) == False:
                #     os.makedirs(path)
                # np.save(path + f'ff_layer1_{num}.npy', fused_feature_list[1].detach().cpu().numpy())
                # np.save(path + f'ff_layer2_{num}.npy', fused_feature_list[2].detach().cpu().numpy())
            else:
                ups.append(x[i])
        if len(ups) > 1:
            x = torch.cat(ups, dim=1)
