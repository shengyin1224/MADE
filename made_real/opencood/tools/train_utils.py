...
def load_saved_model_with_epoch(saved_path, model, epoch_num):
    """
    Load saved model if exiseted
    ...
    # print('resuming by loading epoch %d' % initial_epoch)
    loaded_state_dict = torch.load(os.path.join(saved_path,
                        f'net_epoch{epoch_num}.pth'), map_location='cpu')
    check_missing_key(model.state_dict(), loaded_state_dict)
    model.load_state_dict(loaded_state_dict, strict=False)
...
def create_model(hypes, temperature=20):
    """
    ...
    if model is None:
        print('backbone not found in models folder. Please make sure you '
              ...
              'called %s ignoring upper/lower case' % (model_filename,
                                                       target_model_name))
        exit(0)
    instance = model(backbone_config, temperature)
    return instance
