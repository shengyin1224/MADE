...
class Encoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.fc = nn.Linear(212992, latent_dim)
...
class Decoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.deconv3 = nn.ConvTranspose2d(2*c_hid, c_hid, kernel_size=3, output_padding=1, padding=1, stride=2) # 8x8 => 16x16
        self.deconv4 = nn.Conv2d(c_hid, c_hid, kernel_size=3, padding=0)
        self.deconv5 = nn.ConvTranspose2d(c_hid, num_input_channels, kernel_size=3, output_padding=1, padding=1, stride=2) # 16x16 => 32x32
        self.tanh = nn.Tanh() # The input images is scaled between -1 and 1, hence the output has to be bounded as well

    def forward(self, x):
        x = self.linear(x)
        x = x.reshape(x.shape[0], -1, 13, 32)
        x = self.deconv1(x)
        x = self.act(x)
        x = self.deconv2(x)
        x = self.act(x)
