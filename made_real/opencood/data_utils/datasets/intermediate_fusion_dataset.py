    ...
    class IntermediateFusionDataset(cls):
        ...
        def __getitem__(self, idx):
            ...
            for cav_id, selected_cav_base in base_data_dict.items():
                ...
                # distance_save_path = '/GPFS/data/shengyin/OpencoodV2-Main/OpenCOODv2/outcome/distance/'
                # if cav_id == 1:
                #     np.save(distance_save_path + f'sample{idx}.npy', distance)
        ...
        def collate_batch_train(self, batch):
            ...
            for i in range(len(batch)):
                ...
                if self.load_lidar_file:
                    ...
                    # print(ego_dict['processed_lidar'].keys())
                    # for key in ego_dict['processed_lidar'].keys():
                    #     print(len(ego_dict['processed_lidar'][key]))
                if self.load_camera_file:
                    image_inputs_list.append(ego_dict['image_inputs']) # different cav_num, ego_dict['image_inputs'] is dict.
                
                record_len.append(ego_dict['cav_num'])
                sample_idx_list.append(ego_dict['sample_idx'])
                label_dict_list.append(ego_dict['label_dict'])
                # voxel_num_list.append(ego_dict['label_dict'])
            ...
            if self.load_lidar_file:
                ...
                voxel_shape_list = [len(merged_feature_dict['voxel_features'][ki]) for ki in range(len(merged_feature_dict['voxel_features']))]
                voxel_shape_list = torch.tensor([sum(voxel_shape_list[sum(record_len[:ki]):sum(record_len[:ki+1])]) for ki in range(len(record_len))])

                output_dict['ego'].update({'voxel_shape_list': voxel_shape_list})
                ...
                if not self.heterogeneous or (self.heterogeneous and sum(lidar_agent) != 0):
                    ...
                    # print(processed_lidar_torch_dict['voxel_features'].shape)
        ...
        def post_process(self, data_dict, output_dict):
            ...
            """
            pred_box_tensor, pred_score, pred_box = \
                self.post_processor.post_process(data_dict, output_dict)
            gt_box_tensor = self.post_processor.generate_gt_bbx(data_dict)

            return pred_box_tensor, pred_score, gt_box_tensor, pred_box
