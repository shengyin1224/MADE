...
class VoxelPostprocessor(BasePostprocessor):
    ...
    def post_process(self, data_dict, output_dict):
        ...
        for cav_id, cav_content in data_dict.items():
            ...
            if 'transformation_matrix' in cav_content.keys():
                transformation_matrix = cav_content['transformation_matrix'] # no clean
            else:
                transformation_matrix = torch.from_numpy(np.identity(4)).float().cuda()
        ...
        pred_box3d_tensor = pred_box3d_tensor[index]
    
        # pred_box3d_tensor = pred_box3d_tensor[:500]
        # scores = scores[:500]
        ...
        assert scores.shape[0] == boxes3d.shape[0]

        boxes3d = corner_to_center_torch(pred_box3d_tensor, order='hwl')

        return pred_box3d_tensor, scores, boxes3d
