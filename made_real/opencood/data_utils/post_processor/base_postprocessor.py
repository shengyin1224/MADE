...
class BasePostprocessor(object):
    ...
    def generate_gt_bbx_by_iou(self, data_dict):
        ...
        for cav_id, cav_content in data_dict.items():
            # used to project gt bounding box to ego space
            # object_bbx_center is clean.
            if 'transformation_matrix_clean' in cav_content.keys():
                transformation_matrix = cav_content['transformation_matrix_clean'] # no clean
            else:
                transformation_matrix = torch.from_numpy(np.identity(4)).float().cuda()
            # import ipdb; ipdb.set_trace()
            # if (transformation_matrix - torch.from_numpy(np.identity(4)).float().cuda()).sum() != 0:
