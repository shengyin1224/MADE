...
class PGD(Attack):
            ...
            if self.noise_attack: 
                ...
                for _ in range(self.steps):
                    ...
                    if ext_attack is not None:
                        input_attack = self.model.place_attack(ext_attack, ext_attack_src, ext_attack_tgt, attack_src, attack_tgt)
                    else:
                    ...
                    if self._targeted:
                        ...
                        cost = self.loss(outputs, anchors, reg_loss_mask, reg_targets, labels)
                        if self.attack_classifier:
    ...
    def loss(self,
        ...
        # to corner
        if fg_proposal.any():
            decoded_pred = bev_box_decode_torch(pred_loc[fg_proposal], anchors[fg_proposal]) # (N, 6) [x, y, w, h, sin, cos]
            decoded_pred = rescale_boxes(decoded_pred)
            decoded_target = bev_box_decode_torch(reg_targets[fg_proposal], anchors[fg_proposal]) # (N, 6)
