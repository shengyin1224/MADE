...
RESIDUAL_AE_V2 = "/DB/data/yanghengzhao/adversarial/DAMC/zhenxiang/ae_unet_residual/model_unet/autoencoder.pth"

CALIBRATION_FILE = "/DB/data/yanghengzhao/adversarial/DAMC/yanghengzhao/disco-net/experiments/kde/no-attack/train_emb.npz"


class KDEDetector(nn.Module):
    def __init__(self, checkpoint_path=RESIDUAL_AE_V2):
        super().__init__()
        self.encoder = UNet(in_channels=256, out_channels=256)
        self.encoder.load_state_dict(torch.load(checkpoint_path, map_location="cpu"))

        self.x_train = np.load(CALIBRATION_FILE)['train']
    ...
    def forward(self,
        ...
        embeddings = self.encoder(residual_feat, True)

        # is_attacker = recon_loss > self.threshold
        is_attacker = torch.zeros(embeddings.shape[0], dtype=torch.bool).to(embeddings.device)
        ...
        return com_src, com_tgt, total, correct, \
            ...
            #  "score": recon_loss.detach().cpu().numpy(), 
             "score": is_attacker.float().detach().cpu().numpy(), 
