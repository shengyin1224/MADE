...
import sys
sys.path.append("/DB/data/yanghengzhao/adversarial/DAMC/zhenxiang/ae_unet_residual")
from src.models import UNet, UNet256, UNet128, UNet64
sys.path.pop()
from .utils import label_attacker, rm_com_pair

RESIDUAL_AE_V3 = {
    1024: "/DB/data/yanghengzhao/adversarial/DAMC/zhenxiang/ae_unet_residual/model_unet/autoencoder.pth",
    256: "/DB/data/yanghengzhao/adversarial/DAMC/zhenxiang/ae_unet_residual/model_unet/256/autoencoder_199.pth",
    128: "/DB/data/yanghengzhao/adversarial/DAMC/zhenxiang/ae_unet_residual/model_unet/128/autoencoder_199.pth",
    64: "/DB/data/yanghengzhao/adversarial/DAMC/zhenxiang/ae_unet_residual/model_unet/64/autoencoder_199.pth",
}

CALIBRATION_FILE = "/DB/data/yanghengzhao/adversarial/DAMC/yanghengzhao/disco-net/mreconstruction_loss_validation_v2.npy"


class ResidualAutoEncoderV3ReconstructionLoss(nn.Module):
    def __init__(self, mid_dim=256, checkpoint_path=None):
        super().__init__()
        if mid_dim == 1024:
            self.ae = UNet(in_channels=256, out_channels=256)
        elif mid_dim == 256:
            self.ae = UNet256(in_channels=256, out_channels=256)
        elif mid_dim == 128:
            self.ae = UNet128(in_channels=256, out_channels=256)
        elif mid_dim == 64:
            self.ae = UNet64(in_channels=256, out_channels=256)
        else:
            raise NotImplementedError
        
        if checkpoint_path is None:
            checkpoint_path = RESIDUAL_AE_V3[mid_dim]
...
class ResidualAutoEncoderV3Detector(nn.Module):
    def __init__(self, mid_dim=64, checkpoint_path=None):
        super().__init__()
        self.loss = ResidualAutoEncoderV3ReconstructionLoss(mid_dim, checkpoint_path)
    ...
    def forward(self,
        ...
        is_attacker = recon_loss >= recon_loss.min()  # all true
