...
RESIDUAL_AE_V0 = "/DB/data/yanghengzhao/adversarial/DAMC/zhenxiang/model/autoencoder_512.pth"

CALIBRATION_FILE = "/DB/data/yanghengzhao/adversarial/DAMC/yanghengzhao/disco-net/mreconstruction_loss_validation.npy"


class ResidualAutoEncoderV0ReconstructionLoss(nn.Module):
    def __init__(self, checkpoint_path=RESIDUAL_AE_V0):
        super().__init__()
        self.ae = Autoencoder(base_channel_size=32, latent_dim=512, num_input_channels=256)
...
class ResidualAutoEncoderV0Detector(nn.Module):
    def __init__(self, checkpoint_path=RESIDUAL_AE_V0):
        super().__init__()
        self.loss = ResidualAutoEncoderV0ReconstructionLoss(checkpoint_path)
