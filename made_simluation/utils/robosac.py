...
class HungarianMatcher_ROBOSAC(nn.Module):
    def __init__(self, cost_class = 1, cost_giou = 1):
        super(HungarianMatcher_ROBOSAC, self).__init__()
        ...
        for pred, target in zip(pred_boxes, target_boxes):
            ...
            for i in range(m):
                for j in range(n):
                    ...
                    box_cost[i, j] = 1.0 - cal_giou(target[0]['rot_box'][None, j:j+1], pred[0]['rot_box'][None, i:i+1])[1].cpu().numpy()
